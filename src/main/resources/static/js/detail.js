...
$modifyBtn.addEventListener('click', e=>{
  location.href=`/forums/${pid}/edit`;       // GET http://서버주소 or 서버도메인/products/상품번호/edit
});
...
$listBtn.addEventListener('click',e=>{
  location.href = '/forums';      // GET http://서버주소 or 서버도메인/products
}, false);
...
$delModal.addEventListener('click' , evt=> {
  if($delModal.returnValue == 'ok') {
    location.href = `/forums/${pid}/del`;  // GET http://서버주소 or 서버도메인/forums/상품번호/del
  }
});
